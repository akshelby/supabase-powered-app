...
export function CategoryItemPill({
  ...
  const pillContent = (
    ...
    >
      <motion.div
        className={cn(
          'relative flex items-center gap-2 px-3 py-2.5 sm:px-4 sm:py-3',
        ...
        style={{
          ...
          background: prominent
            ? accentColor === '#25D366' ? 'linear-gradient(135deg,#25D366,#128C7E)' : `linear-gradient(135deg, #ef4444, #b91c1c)`
            : 'hsl(var(--card))',
      ...
      >
        ...
        <div className="flex items-center gap-2 pl-1" style={{ transform: 'skewX(8deg)' }}>
          <Icon
            className={cn(
          ...
          <span
            className={cn(
              'text-[10px] sm:text-[11px] font-bold uppercase tracking-widest leading-tight whitespace-nowrap',
              prominent ? 'text-white' : 'text-foreground'
            )}
          >
            {name}
